...
y_val_dt = dt_pipe.predict(X_val)
...
y_test_scores = dt_pipe.predict_proba(X_test)[:, 1]
...
def predict_fn(data_array):
    ...
    return best_dt_pipe.predict_proba(df)

explainer = shap.KernelExplainer(predict_fn, background)

shap_values = explainer.shap_values(X_test.iloc[:800], nsamples=200)
print("Returned shap_values is a", type(shap_values), "of length", len(shap_values))

all_shap = np.stack(shap_values, axis=0)
print("all_shap.shape →", all_shap.shape) 
...
print("\nTop 10 features by mean |SHAP| (MLP via KernelExplainer):\n")
